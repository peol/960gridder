...
function createObject(name, insertTag, params) {
	...
	for (var i in params) {
		...
		if (i != "noCache") { tmpObj[i] = params[i]; }
	}
...
function Grid() {
	...
	window.gridSettingsDef = {
		urlBase: 'http://960gridder.keep.se/releases/1.2/',
		gColor: '#EEEEEE',
		gColorHover: '#EEEEEE',
		gColumns: 12,
		gOpacity: 0.35,
	...
	this.setVariable = function(variable, value) {
		...
		if (value === true) {
				$('#g-setup-' + variable).attr('checked', 'checked');
		}
		else if (value === false) {
				$('#g-setup-' + variable).removeAttr('checked');
		}
		else {
			$('#g-setup-' + variable).val(value);
		}
